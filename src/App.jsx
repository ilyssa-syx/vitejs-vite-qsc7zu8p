...
const Styles = () => (
  <style>{`
    ...
    .main-title { 
      font-size: 5rem; color: #dffadd; text-shadow: 0 0 30px rgba(223,250,221,0.6); 
      margin: 0; line-height: 1.1; text-align: center;
    }
    .sub-title { 
      font-size: 1.5rem; color: #94a3b8; letter-spacing: 0.2em; margin-top: 20px; text-align: center;
    }
      ...
      if (window.Hands) {
        ...
        hands.onResults((results) => {
          ...
          if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
            ...
            if (window.drawConnectors) window.drawConnectors(ctx, lm, window.HAND_CONNECTIONS, { color: '#00FF00', lineWidth: 1 });
...
const FallingSnow = () => {
  ...
  return (
    <points ref={meshRef}>
      ...
      <pointsMaterial size={0.06} color="#ffffff" transparent opacity={0.7} depthWrite={false} />
    </points>
  );
};
...
const ParticleFloor = () => {
  ...
  return (
    <points>
      <bufferGeometry><bufferAttribute attach="attributes-position" count={count} array={points} itemSize={3} /></bufferGeometry>
      <pointsMaterial size={0.08} color="#8899aa" transparent opacity={0.4} sizeAttenuation={true} depthWrite={false} />
    </points>
  );
};
...
const ParticleStar = ({ powerOn }) => {
  ...
  const points = useMemo(() => {
    const arr = new Float32Array(count * 3);
    const outerRadius = 1.3; const innerRadius = 0.5; const depth = 0.5;
    const vertices = [];
    for (let i = 0; i < 10; i++) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const p1 = { x: 0, y: 0 }; const p2 = vertices[sector]; const p3 = vertices[(sector + 1) % 10];
      let r1 = Math.random(); let r2 = Math.random();
      if (r1 + r2 > 1) { r1 = 1 - r1; r2 = 1 - r2; }
      const a = 1 - r1 - r2;
      const x = a * p1.x + r1 * p2.x + r2 * p3.x;
      const y = a * p1.y + r1 * p2.y + r2 * p3.y;
      arr[i * 3] = x; arr[i * 3 + 1] = y; arr[i * 3 + 2] = (Math.random() - 0.5) * depth;
    }
    return arr;
  }, []);
  useFrame((state, delta) => { if (meshRef.current && powerOn) meshRef.current.rotation.y += delta * 0.2; });
  return (
    <points ref={meshRef}>
      <bufferGeometry><bufferAttribute attach="attributes-position" count={count} array={points} itemSize={3} /></bufferGeometry>
      <pointsMaterial size={0.06} color="#ffffff" transparent opacity={powerOn ? 1.0 : 0.1} blending={THREE.AdditiveBlending} depthWrite={false} />
    </points>
  );
};
...
const SpiralRibbon = ({ powerOn }) => {
  ...
  const points = useMemo(() => {
    const arr = new Float32Array(count * 3);
    const height = 37; const loops = 7; const maxRadius = 16;
    for (let i = 0; i < count; i++) {
      const t = i / count;
      const y = 8 - t * height;
  ...
  useFrame((state, delta) => { if (meshRef.current && powerOn) meshRef.current.rotation.y -= delta * 0.5; });
  return (
    <points ref={meshRef}>
      <bufferGeometry><bufferAttribute attach="attributes-position" count={count} array={points} itemSize={3} /></bufferGeometry>
      <pointsMaterial size={0.25} color="#ffffff" transparent opacity={powerOn ? 0.9 : 0.05} blending={THREE.AdditiveBlending} depthWrite={false} />
    </points>
  );
};
...
const createTextPoints = (text, width = 64, height = 32) => {
  ...
  ctx.fillStyle = '#ffffff'; ctx.strokeStyle = '#ffffff'; ctx.lineWidth = 25;
...
// --- 可变形粒子树/文字 ---
const MorphParticleTree = ({ powerOn, gestureState, recipient, targetMode, treeColor }) => {
  const count = 15000;
  const meshRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (targetMode === 'TEXT') {
        for(let i=0; i<count; i++) {
            targetColors.current[i*3] = 4.0; targetColors.current[i*3+1] = 4.0; targetColors.current[i*3+2] = 4.0;
        }
    } else {
        // 变回树时，使用当前选中的 treeColor 生成颜色
        const newColors = generateTreeColors(count, treeColor);
        for(let i=0; i<count; i++) {
            targetColors.current[i*3] = newColors[i*3]; 
            targetColors.current[i*3+1] = newColors[i*3+1]; 
    ...
    for(let i=0; i<count; i++) {
        ...
        targetPositions.current[i*3+1] = Math.sin(phi) * Math.sin(theta) * r;
        ...
        if (targetMode === 'TEXT') {
            ...
            for(let i=0; i<count; i++) {
                if (i < len) {
                   ...
                   targetPositions.current[i*3] = 0; targetPositions.current[i*3+1] = -200; targetPositions.current[i*3+2] = 0;
                }
            }
        } else {
            // Back to tree (position is static, handled by initialData positions loop logic really)
            // Need to reconstruct tree shape here or just use initialData.pos which is correct
            for(let i=0; i<count; i++) {
  ...
  return (
    <points ref={meshRef}>
      <bufferGeometry>
        <bufferAttribute attach="attributes-position" count={count} array={pointsRef.current} itemSize={3} />
        <bufferAttribute attach="attributes-color" count={count} array={colorsRef.current} itemSize={3} />
      </bufferGeometry>
      <pointsMaterial size={0.12} vertexColors transparent opacity={powerOn ? 0.9 : 0.4} blending={THREE.AdditiveBlending} depthWrite={false} />
    </points>
  );
};
...
const Scene = ({ powerOn, recipient, morphMode, treeColor }) => {
  return (
    <>
      ...
      <OrbitControls enableZoom={false} autoRotate={powerOn && morphMode === 'TREE'} autoRotateSpeed={0.5} minPolarAngle={Math.PI / 3} maxPolarAngle={Math.PI / 1.8} />
      <ambientLight intensity={0.5} color="#b0d0ff" />
      <group position={[0, -9, -85]} scale={[1, 1, 1]}>
        <MorphParticleTree powerOn={powerOn} recipient={recipient} targetMode={morphMode} treeColor={treeColor} />
        <group visible={morphMode === 'TREE'}>
            <SpiralRibbon powerOn={powerOn} />
            <group position={[0, 8.5, 0]}><ParticleStar powerOn={powerOn} /></group>
        </group>
      </group>
      <FallingSnow />
      <ParticleFloor />
      <EffectComposer disableNormalPass>
        <Bloom luminanceThreshold={0.8} luminanceSmoothing={0.5} mipmapBlur intensity={1.5} radius={0.3} />
...
export default function App() {
  ...
  const [treeColor, setTreeColor] = useState(TREE_COLORS[0].hex); // 默认绿色
  ...
  const handleGesture = (data) => {
    ...
    if (now - lastModeChangeTime.current > 1500) {
        ...
        // <--- 修复点：Back to Tree 的判定放宽 --->
        // 只要检测到 PINCH 且当前是 TEXT 模式，就触发
  ...
  return (
    <>
      ...
      <div className="ui-container">
        <div className="start-screen" style={{ opacity: started ? 0 : 1, pointerEvents: started ? 'none' : 'auto' }}>
          <h1 className="main-title lux-font">Merry Christmas</h1>
          <p className="sub-title inter-font">TO: {recipient}</p>
          <button className="enter-btn" onClick={() => setStarted(true)}>ENTER</button>
        </div>
        ...
        >
            ...
            >
                <div className="inter-font" style={{ fontSize: '10px', color: '#aaa', letterSpacing: '2px' }}>PINCH & PULL (OR CLICK)</div>
                <div className={`bulb-icon ${powerOn ? 'on' : ''}`}>
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={powerOn ? '#fff' : '#fff'} strokeWidth="2">
        ...
        <button className="morph-btn" style={{ opacity: started ? 1 : 0 }} onClick={() => setMorphMode(prev => prev === 'TREE' ? 'TEXT' : 'TREE')}>
            {morphMode === 'TREE' ? 'SHOW WISHES' : 'BACK TO TREE'}
        </button>

        <div className="camera-box" style={{ opacity: started ? 1 : 0, transition: 'opacity 1s' }}>
          <video ref={videoRef} style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.6 }} autoPlay playsInline muted />
          <canvas ref={canvasRef} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%' }} width={320} height={240} />
        </div>
      </div>

      <Canvas camera={{ position: [0, 0, 10] }} gl={{ antialias: false, toneMapping: THREE.ReinhardToneMapping }}>
        <Suspense fallback={null}>
          {started && <Scene powerOn={powerOn} recipient={recipient} morphMode={morphMode} treeColor={treeColor} gestureState={gestureState} />}
        </Suspense>
      </Canvas>
    </>
  );
